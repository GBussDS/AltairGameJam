...
func _physics_process(delta):
	...
	if draggable and not dragging:
		if get_parent().get_parent().is_dragging == -1 and Input.is_action_just_pressed("click"):
			get_parent().get_parent().is_dragging = num
			# Move para o final da lista para ser desenhado por cima dos outros
			get_parent().get_parent().move_child(self, -1)
			z_index = 1
	...
	if Input.is_action_pressed("click") and get_parent().get_parent().is_dragging == num:
	...
	elif Input.is_action_just_released("click") and get_parent().get_parent().is_dragging == num:
		var duration
		if pickup_tween and pickup_tween.is_running():
			duration = pickup_tween.get_total_elapsed_time()
			pickup_tween.kill()
		...
		dragging = false
		get_parent().get_parent().is_dragging = -1

func _on_area_2d_mouse_entered():
	if get_parent().get_parent().is_dragging == -1:
		draggable = true
